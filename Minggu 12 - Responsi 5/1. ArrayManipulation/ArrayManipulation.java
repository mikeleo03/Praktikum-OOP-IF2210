...
class ArrayManipulation {
    ...
    public static <T extends Comparable<T>> T[] mergeAndSortArrays(T[] arr1, T[] arr2) {
        ...
        // Salin
        T[] arrayJoin = Arrays.copyOf(arr1, totalLength);
        // Merge
        System.arraycopy(arr2, 0, arrayJoin, arr1.length, arr2.length);
        // Sorting
    ...
    public static <T> T[] mapArray(T[] arr, Function<T, T> mapper) {
        // Inisiasi array
        T[] arrayresult = Arrays.copyOf(arr, arr.length);
        // Proses pemetaan dengna mapper
        for (int i = 0; i < arr.length; i++) {
            arrayresult[i] = mapper.apply(arr[i]);
        }
        // Kembalikan hasil
        return arrayresult;
    }
